...
wrapping performance, and homomorphic rate-limit subdivision. The vendor-offline architecture
awaits resolution of the MAC primitive problem.
\end{abstract}
...
The key derivation primitive. Given master key $K_m$ and a structured payload, the
derived key is computed as:
...
computationally infeasible: possession of $dk$ reveals nothing about $K_m$. Token
generation runs on-chain via homomorphic MAC on the CoFHE coprocessor. Validation
...
\draw[msg] (\xB,-3.9) to[out=15,in=-15,looseness=5] (\xB,-4.35);
\node[steplbl,right] at (\xB+0.25,-4.1) {Homomorphic MAC via CoFHE};
...
processing (no network hop). Phase II token generation runs entirely on-chain via
CoFHE; the vendor is offline after initial provisioning.}
\label{fig:protocol}
\end{figure*}
...
where $T_{enc}$ is an encrypted token ciphertext computed entirely on-chain via the
CoFHE coprocessor without vendor involvement.
...
The following components form the foundation of the vendor-offline architecture:
\begin{itemize}
\item \texttt{euint256} master key storage on Fhenix with ACL-gated
      vendor-only decryption
\item On-chain keccak256 commitment storage and use counter
\item Homomorphic MAC token generation via CoFHE coprocessor
\item TSN re-encryption delivery for buyer public keys
\item USDC payment integration via Circle
\item Vendor marketplace registry with SLA parameter management
\end{itemize}

\textbf{Blocked on:} FHE-friendly MAC primitive selection (Section~\ref{sec:open}).

\subsection{Phase 2: DePIN Network}
\begin{itemize}
\item Production DePIN node network with EigenLayer staking
\item Local HMAC validation and request forwarding
\item Batched Merkle usage reporting
\item On-chain slashing with fraud proof verification
\end{itemize}

\subsection{Phase 3: Market Features}
\begin{itemize}
\item Resale market (blocked on homomorphic subdivision research)
\item Multi-chain deployment via LayerZero CCIP
\item Advanced compliance gating with identity attestation providers
\end{itemize}
...
The vendor-offline architecture, in which vendors require no online infrastructure after
initial provisioning, awaits resolution of the FHE-friendly MAC primitive selection problem.
