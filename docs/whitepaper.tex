...
$K_m$ using \texttt{euint256} with access-control-list (ACL) gating on Fhenix L2, (2) the
...
Zama's TFHE library \cite{zama_tfhe} and the Fhenix L2 \cite{fhenix} represent the
...
FHE-402 follows the ``Glue and Coprocessor'' architecture. The Ethereum-compatible
Layer 2 (Fhenix L2) handles business logic and state management --- the ``glue'' ---
...
\textbf{On-Chain Layer (Fhenix L2):} Built on the Arbitrum Nitro stack, the on-chain
layer manages encrypted state using the \texttt{FHE.sol} library. It handles vendor
registration, buyer payments in USDC, token commitment storage via keccak256, rate
limit enforcement at issuance time, and the vendor marketplace registry.
...
\begin{scope}[on background layer]
  \node[draw=nodegray!42, fill=nodegray!7, rounded corners=6pt,
        ...
        label=above:{\small\textbf{Fhenix L2 --- Blockchain}}]
        (chain) {};
\end{scope}
...
\node[actorbox=nodeblue]   at (\xB,0) {\textbf{Fhenix L2}\\Blockchain};
...
\item The Fhenix L2 consensus mechanism is not captured.
...
The PoC implementation is fully buildable with current tooling on Fhenix L2. The full
